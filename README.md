...
上記に関しては、npm の各パッケージの詳細URLが  `https://www.npmjs.com/package/{パッケージ名}` になっているという規則性を見つけ出し、 テーブルの「技術」カラムを npm の 各パッケージの `link` にすることで、パッケージの更新状態の確認やGithubのページに遷移する為のアクションを減らすことが容易になりました。  
